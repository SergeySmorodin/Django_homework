...
```bash
cd 2.1-databases\work_with_database
```
...
- Команда для создания БД на сервере Postgres

```bash
createdb -U postgres netology_import_phones
```

- Команда для удаления БД на сервере Postgres

```bash
psql -d netology_import_phones -c "TRUNCATE TABLE phones_phone;" 
dropdb -U postgres netology_import_phones
```
